...
const Login = () => {
  ...
  const validate = (values) => {
    ...
    errors.email = user.email ? (
      ...
      (/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/).test(user.email) ? '' : 'Please, Provide valid email !') : 'Email is Required !';

    errors.password = user.password ? (user.password.length < 5 ? 'Password must include more than 5 char' : '') : 'Password is Required !';
